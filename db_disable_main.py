...
def time(ms):
    ...
    # Это понадобится для того, чтобы шло время, когда проигрывается трек
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def upload_tracks(self):
        ...
        result = con.cursor().execute("SELECT * FROM albums").fetchall()
        for i in result:
            print(i)
            self.playlist.addMedia(QMediaContent(i))
        con.close()

    def save_tracks(self, id):
        albums = [(id)]
        con = sqlite3.connect("music.db")
        con.cursor().execute("INSERT INTO albums VALUES (?)", albums)
        con.commit()
        con.close()
    ...
    def dropEvent(self, file):
        # Добавляет в список вопсроизведения треки
